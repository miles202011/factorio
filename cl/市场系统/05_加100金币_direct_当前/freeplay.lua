...
script.on_event(defines.events.on_player_joined_game, function(event)
  ...
  if storage.gifted_players[player.name] then
    game.print("[debug] " .. player.name .. " 已送过，跳过")
    return
  end
  storage.gifted_players[player.name] = true
  if storage.created_items then
    util.insert_safe(player, storage.created_items)
    player.print("[color=red]<服务器>" .. player.name .. "首次加入本局游戏赠送100金币，之后每分钟赠送1金币[/color]")
  else
    game.print("[debug] storage.created_items 为 nil，无法发放")
  end
end)
