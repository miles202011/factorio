...
local function set_raffle()
	storage.rocks_yield_ore_veins.raffle = {}
	for _, t in pairs(get_chances()) do
		for x = 1, t[2], 1 do
			table.insert(storage.rocks_yield_ore_veins.raffle, t[1])
		end			
	end
	...
	storage.rocks_yield_ore_veins.mixed_ores = {"iron-ore", "copper-ore", "stone", "coal"}
end

local function get_amount(position)
	local distance_to_center = math.sqrt(position.x^2 + position.y^2) * 2 + 1500
	distance_to_center = distance_to_center * storage.rocks_yield_ore_veins.amount_modifier
		...
		for i = 1, 4, 1 do
			...
			if surface.can_place_entity({name = "coal", position = p, amount = 1}) then
				if not ore_positions[p.x .. "_" .. p.y] then
					...
					local name = ore
					if ore == "mixed" then name = storage.rocks_yield_ore_veins.mixed_ores[math_random(1, #storage.rocks_yield_ore_veins.mixed_ores)] end
...
local function ore_vein(event)
	...
	if ore == "mixed" then
		ore_entities = {{name = storage.rocks_yield_ore_veins.mixed_ores[math_random(1, #storage.rocks_yield_ore_veins.mixed_ores)], position = {x = event.entity.position.x, y = event.entity.position.y}, amount = get_amount(event.entity.position)}} 
	end
...
local function on_init()
	storage.rocks_yield_ore_veins = {}
	storage.rocks_yield_ore_veins.raffle = {}
	storage.rocks_yield_ore_veins.mixed_ores = {}
	storage.rocks_yield_ore_veins.chance = 768
	storage.rocks_yield_ore_veins.amount_modifier = 1
	set_raffle()
end
